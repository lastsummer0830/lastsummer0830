...
행성을 하나씩 밟아 진도를 쌓는 학습 로드맵 서비스입니다. 학습 도메인의 `Controller → Service → Mapper → View` 세로 전 구간을 맡아, 도메인마다 흩어지던 단계 잠금해제를 **`UnlockService` 하나로 통일**해 정합성을 잡았습니다. 우주 행성 콘셉트의 학습 로드맵 화면도 디자이너 없이 직접 설계·구현했습니다.
